# 📊 Análisis de Tasas de Interés (Ahorros y Créditos)

![MicrosoftSQLServer](https://img.shields.io/badge/Microsoft%20SQL%20Server-CC2927?style=for-the-badge&logo=microsoft%20sql%20server&logoColor=white)  ![Power Bi](https://img.shields.io/badge/power_bi-F2C811?style=for-the-badge&logo=powerbi&logoColor=black) ![](https://badges.pufler.dev/visits/raulch3/TasaBancarias?style=flat-square&logo=Github)

## 📌 Descripción

La importancia de conocer las tasas de interés en el mercado financiero, tanto en productos de **ahorro (pasivos)** como de **crédito (activos)**, radica en que permite a los usuarios tomar mejores decisiones financieras.

Este análisis facilita la comparación entre instituciones bancarias, brindando mayor claridad tanto a:
- 💰 Ahorradores
- 🏦 Sujetos de crédito

---

## 🎯 Objetivo

- Analizar las tasas de interés del mercado
- Calcular promedios de tasas activas y pasivas
- Determinar el **spread financiero**
- Identificar niveles de rentabilidad entre instituciones

---

## 🧠 Concepto Clave: Spread

El **spread** es la diferencia entre la tasa activa y la tasa pasiva:

```text
Spread = Tasa Activa Promedio - Tasa Pasiva Promedio
```
# Visto desde SSM22
<img width="1366" height="768" alt="image" src="https://github.com/user-attachments/assets/8a58fce5-e970-4df7-a5e7-d5356ce15664" />

# VIsualización de datos desde PowerBI (Básico)
<img width="1366" height="768" alt="image" src="https://github.com/user-attachments/assets/9de24af4-5d2d-4e3f-a5da-b3569528e722" />
